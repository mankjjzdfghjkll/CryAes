using System;

namespace CryAes  // Changé de Aes à CryAes pour harmoniser
{
    public class Key
    {
        private static byte[] rcon = 
        {
            
        };
        
        private byte[,] _w;
        private int _size;
        private int _nr;  // Nombre de rounds

        public int Size
        {
            get { return _size; }
            set { _size = value; }
        }

        public int Nr
        {
            get { return _nr; }
        }

        public byte[,] w
        {
            get { return _w; }
            set { _w = value; }
        }

        public Key(byte[] b)
        {
            _size = b.Length;
            int nk = _size / 4;  // key length (in words): 4/6/8 for 128/192/256-bit keys
            _nr = nk + 6;        // Nombre de rounds: 10, 12, ou 14
            _w = new byte[4, 4 * (_nr + 1)];
            
            for (int i = 0; i < nk; i++)
            {
                _w[0, i] = b[4 * i];
                _w[1, i] = b[4 * i + 1];
                _w[2, i] = b[4 * i + 2];
                _w[3, i] = b[4 * i + 3];
            }

            for (int i = nk; i < (4 * (_nr + 1)); i++)
            {
                byte[] temp = new byte[4];
                temp[0] = _w[0, i - 1];
                temp[1] = _w[1, i - 1];
                temp[2] = _w[2, i - 1];
                temp[3] = _w[3, i - 1];
                
                if ((i % nk) == 0)
                {
                    temp = rowShift(temp);
                    temp = subByte(temp);
                    temp[0] ^= rcon[i / nk];
                }
                
                if ((nk == 8) && ((i % nk) == 4))
                {
                    temp = subByte(temp);
                }
                
                _w[0, i] = (byte)(_w[0, i - nk] ^ temp[0]);
                _w[1, i] = (byte)(_w[1, i - nk] ^ temp[1]);
                _w[2, i] = (byte)(_w[2, i - nk] ^ temp[2]);
                _w[3, i] = (byte)(_w[3, i - nk] ^ temp[3]);
            }
        }

        private byte[] subByte(byte[] b)
        {
            for (int i = 0; i < b.Length; i++)
            {
                b[i] = Sbox.sbox[b[i]];
            }
            return (b);
        }

        private byte[] rowShift(byte[] b)
        {
            byte[] temp = new byte[b.Length];
            for (int i = 0; i < b.Length - 1; i++)
            {
                temp[i] = b[i + 1];
            }
            temp[b.Length - 1] = b[0];
            return (temp);
        }
        
        // Méthode corrigée pour obtenir la clé de round
        public byte[,] GetRoundKey(int round)
        {
            byte[,] roundKey = new byte[4, 4];
            int startIndex = round * 4;
            
            for (int col = 0; col < 4; col++)
            {
                for (int row = 0; row < 4; row++)
                {
                    roundKey[row, col] = _w[row, startIndex + col];
                }
            }
            return roundKey;
        }
        
        public override string ToString()
        {
            string s = "";
            for (int i = 0; i < _w.GetLength(0); i++)
            {
                for (int j = 0; j < _w.GetLength(1); j++)
                {
                    s += _w[i, j].ToString("X2") + " ";
                }
                s += "\n";
            }
            return (s);
        }
    }
}